...
func TestSumupStmtpng2tsvToPoppit(t *testing.T) {
	...
	expectedCmd := `echo ./stmtpng2tsv -output "/workspace/incoming/SumUp-Statement-Aug-26.tsv" "/workspace/incoming/SumUp-Statement-Aug-26-?.png"`
...
func TestSumupStmtpng2tsvToPoppitSupportsInputFileWithSpaces(t *testing.T) {
	...
	expectedCmd := `echo ./stmtpng2tsv -output "/workspace/incoming/My SumUp-Statement-Aug-26.tsv" "/workspace/incoming/My SumUp-Statement-Aug-26-?.png"`
	if msg.Commands[0] != expectedCmd {
		t.Fatalf("unexpected command:\nexpected: %q\ngot:      %q", expectedCmd, msg.Commands[0])
	}
}
